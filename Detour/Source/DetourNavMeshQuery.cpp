	...
	dtPolyRef* randomRef, float* randomPt) const
{
	...
	const dtMeshTile* randomTile{};
	const dtPoly* randomPoly{};
...
// ///指定された位置がポリゴンのxz境界の外にある場合、＃DT_FAILURE | #DT_INVALID_PARAM を返します。
///
dtStatus dtNavMeshQuery::getPolyHeight(dtPolyRef ref, const float* pos, float* height) const
{
	...
	// We used to return success for offmesh connections, but the getPolyHeight in DetourNavMesh does not do this,
	// so special case it here.
	// 以前はオフメッシュ接続の成功を返していましたが、DetourNavMeshのgetPolyHeightはこれを行いません。
	// したがって、ここでは特殊なケースです。
	...
	*pathCount = 0;
	...
	while (!m_openList->empty())
	{
		...
		for (unsigned int i = bestPoly->firstLink; i != DT_NULL_LINK; i = bestTile->links[i].next)
		{
			...
			if (bestTile->links[i].side != 0xff) crossSide = bestTile->links[i].side >> 1;
	...
	*pathCount = 0;
	...
	else
	{
		...
		do
		{
			...
			// 親が隣接していないかどうか（つまり、レイキャストのショートカットが原因であるかどうか）を追跡し、
			// 反転したパスのノードに保存します
			int nextRay = node->flags & DT_NODE_PARENT_DETACHED;
			node->flags = (node->flags & ~DT_NODE_PARENT_DETACHED) | prevRay;
			prevRay = nextRay;
	...
	dtRaycastHit* hit, dtPolyRef prevRef) const
{
	...
	// API入力はすでにチェックされています。内部データのチェックはスキップしてください。
	curRef = startRef;
	tile = 0;
	poly = 0;
	...
	while (curRef)
	{
		...
		{
			// ポリゴンをヒットできませんでした。古いtを保持し、ヒットを報告します。
			hit->pathCount = n;
			return status;
		}
		...
		for (unsigned int i = poly->firstLink; i != DT_NULL_LINK; i = tile->links[i].next)
		{
			...
			// リンクがエッジ全体にまたがっているかどうかを確認し、受け入れます。
			if (link->bmin == 0 && link->bmax == 255)
			{
				nextRef = link->ref;
				break;
		...
		if (options & DT_RAYCAST_USE_COSTS)
		{
			...
			// ポリゴンの最も遠い端の交点を計算し、高さを修正します（レイキャストは2Dで移動するため）
