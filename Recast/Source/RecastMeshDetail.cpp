...
namespace
{
	constexpr unsigned RC_UNSET_HEIGHT = 0xffff;
	...
	inline constexpr float vdot2(const float* a, const float* b)
	{
		return a[0] * b[0] + a[2] * b[2];
	}

	inline constexpr float vdistSq2(const float* p, const float* q)
	{
	...
	inline constexpr float vcross2(const float* p1, const float* p2, const float* p3)
	{
	...
	bool circumCircle(const float* p1, const float* p2, const float* p3,
		float* c, float& r)
	{
	...
	inline float distPtTri(const float* p, const float* a, const float* b, const float* c)
	{
	...
	inline constexpr float distancePtSeg(const float* pt, const float* p, const float* q)
	{
		float pqx = q[0] - p[0];
		float pqy = q[1] - p[1];
		float pqz = q[2] - p[2];
	...
	inline constexpr float distancePtSeg2d(const float* pt, const float* p, const float* q)
	{
		float pqx = q[0] - p[0];
		float pqz = q[2] - p[2];
		float dx = pt[0] - p[0];
	...
	float distToTriMesh(const float* p, const float* verts, const int /*nverts*/, const int* tris, const int ntris)
	{
	...
	float distToPoly(int nvert, const float* verts, const float* p)
	{
		float dmin = (std::numeric_limits<float>::max)();
		int i, j, c = 0;
	...
	unsigned short getHeight(const float fx, const float fy, const float fz,
	...
	int findEdge(const int* edges, int nedges, int s, int t)
	{
	...
	int addEdge(rcContext* ctx, int* edges, int& nedges, const int maxEdges, int s, int t, int l, int r)
	{
		if (nedges >= maxEdges)
		{
	...
	inline constexpr void updateLeftFace(int* e, int s, int t, int f)
	{
	...
	inline constexpr int overlapSegSeg2d(const float* a, const float* b, const float* c, const float* d)
	{
	...
	bool overlapEdges(const float* pts, const int* edges, int nedges, int s1, int t1)
	{
		for (int i = 0; i < nedges; ++i)
		{
	...
	void completeFacet(rcContext* ctx, const float* pts, int npts, int* edges, int& nedges,
		const int maxEdges, int& nfaces, int e)
	{
	...
	// ポリゴンの最小延長を計算します。
	float polyMinExtent(const float* verts, const int nverts)
	{
	...
	// ポリゴン中心のシード配列
	void seedArrayWithPolyCenter(rcContext* ctx, const rcCompactHeightfield& chf,
	...
	void push3(rcIntArray& queue, int v1, int v2, int v3)
	{
	...
	void getHeightData(rcContext* ctx, const rcCompactHeightfield& chf,
		const unsigned short* poly, const int npoly,
		const unsigned short* verts, const int bs,
	...
	unsigned char getTriFlags(const float* va, const float* vb, const float* vc,
		const float* vpoly, const int npoly)
	{
