...
namespace
{
	inline bool overlapBounds(const std::array<float, 3>& amin, const std::array<float, 3>& amax,
	...
	rcSpan* allocSpan(rcHeightfield& hf)
	{
		...
		// メモリが不足している場合は、新しいページを割り当てて空きリストを更新します。
		if (!hf.freelist || !hf.freelist->next)
		{
			...
			auto head = pool->items.begin();
			auto it = pool->items.end();
	...
	// 凸多角形を線の両側で2つの凸多角形に分割します
	inline void dividePoly(const float* in, int nin,
	...
	inline bool rasterizeTri(const float* v0, const float* v1, const float* v2,
		...
		const int flagMergeThr)
	{
		...
		// 三角形の境界ボックスを計算します。
		rcVcopy(tmin.data(), v0);
		rcVcopy(tmax.data(), v0);
		rcVmin(tmin.data(), v1);
		rcVmin(tmin.data(), v2);
		rcVmax(tmax.data(), v1);
		rcVmax(tmax.data(), v2);
...
/// 三角形がheightfieldグリッドに重ならない場合、スパンは追加されません。
///
/// @see rcHeightfield
