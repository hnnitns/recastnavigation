...
void NavMeshTesterTool::handleToggle()
{
	...
	if (m_pathIterNum == 0)
	{
		m_navQuery->findPath(m_startRef, m_endRef, m_spos, m_epos, &m_filter, m_polys.data(), &m_npolys, MAX_POLYS);
		m_nsmoothPath = 0;
	...
	dtVcopy(m_steerPos, steerPos.data());
	...
	dtVsub(delta.data(), steerPos.data(), m_iterPos);
	...
	if (endOfPath && inRange(m_iterPos, steerPos.data(), SLOP, 1.f))
	{
		// Reached end of path.
...
void NavMeshTesterTool::recalc()
{
	...
	if (m_toolMode == TOOLMODE_PATHFIND_FOLLOW)
	{
		...
		if (m_sposSet && m_eposSet && m_startRef && m_endRef)
		{
			...
			if (m_npolys)
			{
				...
				m_navQuery->closestPointOnPoly(m_startRef, m_spos, iterPos.data(), 0);
	...
	else if (m_toolMode == TOOLMODE_RAYCAST)
	{
		...
		if (m_sposSet && m_eposSet && m_startRef)
		{
			...
			m_navQuery->raycast(m_startRef, m_spos, m_epos, &m_filter, &t, m_hitNormal, m_polys.data(), &m_npolys, MAX_POLYS);
			if (t > 1)
			{
				// No hit
